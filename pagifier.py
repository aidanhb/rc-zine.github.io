...
def get_layout(content_n, offset, per_page):
    ...
    for i in range(0, len(fronts), per_page):
        front = tuple([tuple(fronts[j]) for j in range(i, i + per_page)])
        back = tuple([tuple(backs[j]) for j in range(i, i + per_page)])
